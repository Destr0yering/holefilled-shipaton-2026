...
export default function App() {
  ...
  const fill=()=>{
    ...
    if(result.ok){
      ...
      setAudit(x=>[...x,{id:`${scenario.id}-a5`,at:'Now',actor:'ai',label:'Approved assistance offered',detail:`The assistance is within ${scenario.organizationLabel}'s configured policy.`},{id:`${scenario.id}-a6`,at:'Now',actor:'worker',label:'Shift accepted',detail:`${winner?.name??'The selected coworker'} accepted; competing offers closed atomically.`},{id:`${scenario.id}-a7`,at:'Now',actor:'system',label:'HoleFilled',detail:'Coverage confirmed with transportation and ETA.'}]);
  ...
  return <SafeAreaView style={s.safe}><StatusBar style="dark" />
    ...
    <ScrollView contentContainerStyle={[s.page,compact&&s.pageCompact]}>
      {screen==='dashboard'&&<>
        <View style={s.scenarioPanel}><Text accessibilityRole="header" style={s.scenarioTitle}>Choose an employer use case</Text><Text style={s.small}>Each scenario uses fictional organizations and employer-controlled internal worker pools.</Text><View style={s.scenarioTabs}>{demoScenarios.map(item=><Pressable key={item.id} accessibilityRole="button" accessibilityState={{selected:item.id===scenario.id}} onPress={()=>selectScenario(item)} style={[s.scenarioTab,item.id===scenario.id&&s.scenarioTabActive]}><Text style={[s.scenarioTabText,item.id===scenario.id&&s.scenarioTabTextActive]}>{item.hole.role.split(' — ')[0]}</Text><Text style={[s.scenarioSector,item.id===scenario.id&&s.scenarioTabTextActive]}>{item.sector}</Text></Pressable>)}</View></View>
        <View style={[s.hero,compact&&s.stack]}><View style={[s.grow,compact&&s.growCompact]}><Text style={s.eyebrow}>{scenario.organizationLabel.toUpperCase()} • {scenario.operationsLabel.toUpperCase()}</Text><Text accessibilityRole="header" style={[s.h1,compact&&s.h1Compact]}>{scenario.headline}</Text><Text style={s.lead}>Three qualified coworkers are ready for a policy-controlled outreach wave.</Text></View><View style={[s.metric,compact&&s.metricCompact]}><Text style={s.metricLabel}>Coverage risk</Text><Text style={s.metricBig}>{scenario.escalationMinutes} min</Text><Text style={s.metricHint}>before escalation</Text></View></View>
        <View style={s.card}><View style={[s.between,compact&&s.stack]}><View style={[s.grow,compact&&s.growCompact]}><View style={s.inline}><Pill label="Critical" tone="red"/><Text style={s.mono}>{hole.externalId}</Text></View><Text accessibilityRole="header" style={s.h2}>{hole.role}</Text><Text style={s.body}>{hole.worksite} · {hole.startsAt}–{hole.endsAt}</Text></View><Pill label={`${candidates.length} qualified fillers`}/></View><View style={s.rule}/><View style={[s.grid,compact&&s.stack]}><Info k="Required" v={scenario.requiredLabel}/><Info k="Auto incentive" v={`Up to $${(hole.policy.autonomousLimitCents/100).toFixed(0)}`}/><Info k="Transport support" v={`Up to $${(hole.policy.rideCreditLimitCents/100).toFixed(0)}`}/></View><View style={[s.actions,compact&&s.stack]}><Button label="Start HoleFilled outreach" onPress={begin}/><Button label="Review incident" onPress={()=>setScreen('incident')} secondary/></View></View>
      </>}
